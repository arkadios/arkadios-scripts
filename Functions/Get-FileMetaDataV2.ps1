...
Function Get-FileMetaDataV2
{
 ...
 if ($PSVersionTable.PSEdition -ne 'Desktop' -and -not $IsWindows) {
     Write-Error "Get-FileMetaDataV2 is only supported on Windows (requires Shell.Application COM object)."
     return
 }
         ...
         if($objFolder.getDetailsOf($File, $a))
           {
			...
			{
				# if property say its date lets try to convert it
				[ref]$myOutput = Get-Date
				if([datetime]::TryParse($($objFolder.getDetailsOf($File, $a)), $myOutput)){
					$propertyValue = $myOutput				
				}
				else
				{
					$propertyValue = $($objFolder.getDetailsOf($File, $a))
				}
			}
			else {
				$propertyValue = $($objFolder.getDetailsOf($File, $a))
