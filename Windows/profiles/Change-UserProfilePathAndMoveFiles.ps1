...
foreach ($USER_PROFILE in $USER_PROFILES) {
    ...
    If ($ACCOUNT -eq "ALL" -or $USER -eq $ACCOUNT)
    {
		...
		read-host "Continue? (ctrl-c to stop / enter to continue)"
		...
		try{
			...
			$continue = $true
			write-host -f green "- DONE Copying."
		}
		catch{
			write-host -f red "Shit happend! Error: '$_'"
		}
		
        # this code fuck things up!!!!!!!!!!!!!
		# if($continue){
		# 	# Change the profile path in the registry

		# 	Set-ItemProperty -LiteralPath $REGISTRY -Name ProfileImagePath -Value $NEW_PROFILEPATH
		# 	Write-Host "- Modified Windows registry (ProfileImagePath) to '$NEW_PROFILEPATH'"
			

		# }
		# else{
		# 	write-host -f "- Registry not changed!"
		# }
        Write-Host "- Done!"
        Write-Host "-------------------------------"		
    }
